...
def run(
    ...
    model_version: Optional[str] = typer.Option(
        None,
        "--model-version",
        help="Optional model version label",
    ),
) -> None:
    ...
    if task not in task_names:
        console.print(
            f"[bold red]Error:[/bold red] unknown task [cyan]{task}[/cyan]. "
            f"Available: {', '.join(task_names) or '(none)'}",
        )
        raise typer.Exit(code=1)

    if provider not in _ALLOWED_PROVIDERS:
        console.print(
            f"[bold red]Error:[/bold red] provider must be one of "
            f"{', '.join(sorted(_ALLOWED_PROVIDERS))}; got [cyan]{provider}[/cyan]",
        )
        raise typer.Exit(code=1)
...
def list_runs_cmd(limit: int = typer.Option(20, "--limit", min=1)) -> None:
    ...
    for row in rows:
        ...
        if total:
            try:
                pr = f"{int(passed or 0)}/{int(total)}"
...
@app.command("report")
def report_cmd(
    run_id: str = typer.Option(..., "--run-id", help="Eval run UUID"),
) -> None:
