...
from evalforge.llm.ollama import OllamaProvider
...
class LLMRouter:
    async def complete(
        ...
        try:
            ...
            if provider == "ollama":
                return await OllamaProvider().complete(prompt, model, max_tokens)
            return await GeminiProvider().complete(prompt, model, max_tokens)
        except httpx.HTTPError as exc:
            logger.error("LLM HTTP error [%s/%s]: %s", provider, model, exc, exc_info=True)
